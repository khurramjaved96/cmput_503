...
class AprilTagDetector(DTROS):
    def __init__(self):
        ...
        self.teleport = rospy.ServiceProxy('/csc22938/deadreckoning_node/teleport', ODO)
        ...
        self._cinfo_sub = rospy.Subscriber("/csc22938/camera_node/camera_info", CameraInfo, self._cinfo_cb, queue_size=1)
        # create publisher
    ...
    def _detect(self, detector_id, msg):
        ...
        for tag in tags:
            ...
            try:
                ...
                trans = self.buffer.lookup_transform("world", "tag/{:s}".format(str(tag.tag_id)), rospy.Time(0),
                                                     rospy.Duration(1.0))

                self._tf_bcaster2.sendTransform(trans)


                trans = self.buffer.lookup_transform_full("tag/{:s}".format(str(tag.tag_id)),  rospy.Time(0), 'csc22938/footprint',rospy.Time(0), 'world',rospy.Duration(1.0) )

                t = trans
                i = apriltag_list.index(tag.tag_id)
                # t = TransformStamped()
                # t = TransformStamped()

                t.header.frame_id = "at_" + str(apriltag_name_list[i]) + "_static"
                t.child_frame_id = "temped"

                self._tf_bcaster2.sendTransform(trans)

                trans = self.buffer.lookup_transform("world", "temped", rospy.Time(0),
                                                     rospy.Duration(1.0))
                ...
                self.teleport(pos.x, pos.y, pos.z, yaw)
                # self.teleport()
        ...
        if detec is not None:
            ...
            # t.child_frame_id = "detected_april_tag"
