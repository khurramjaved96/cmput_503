...
int TARGET_PARK = PARK_FAR_RIGHT;
...
public:
  ...
  static void imageCallback(const sensor_msgs::ImageConstPtr &msg) {
    ...
    if (LaneControl::CURRENT_STATE == GO_STRAIGH_STATE) {
      ...
      if (right) {
        control_msg.vel_right = 0.06;
        control_msg.vel_left = 0.55;
      } else if (left) {
        control_msg.vel_right = 0.55;
        control_msg.vel_left = 0.06;
      } else {
      ...
      if (right) {
        ...
        control_msg.vel_left = 0.50;
        LaneControl::lane_control_pub.publish(control_msg);
      } else if (left) {
        control_msg.vel_right = 0.50;
...
int main(int argc, char **argv) {
  ...
  image_transport::Subscriber sub = it.subscribe("/csc22939/camera_node/image",
